...
const UI = {
  ...
  _montarLogSuceso(carta, logEl) {
    ...
    } else if (carta._resultado_activar_pnj) {
      const r = carta._resultado_activar_pnj;
      logEl.style.display = 'block';
      if (r.hasta) {
        logEl.innerHTML = `<p style="font-family:var(--f3);font-size:1.05rem;color:#e8dcc8;line-height:1.7;"><strong>${r.pnjNombre}</strong> se desplaza de <em>${r.nomDesde}</em> a <em>${r.nomHasta}</em>.</p>`;
      } else {
