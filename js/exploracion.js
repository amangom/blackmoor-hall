...
function getCartasDisponibles(losetaId) {
  ...
  return _cartasExpl.cartas
    .filter(c => c.loseta_id === losetaId && !jugadas.includes(c.id))
    .sort((a, b) => {
      // Ordenar por el número de loseta que aparece en el reverso: "Vestíbulo — 1" → 1
      const numA = parseInt((a.reverso || '').match(/—\s*(\d+)/)?.[1] || '0');
      const numB = parseInt((b.reverso || '').match(/—\s*(\d+)/)?.[1] || '0');
      return numA - numB;
    });
}
  ...
  for (const ef of efectos) {
    switch (ef.tipo) {
      ...
      case 'bonus_interpretacion':
        ...
        if (pistaBonusId) {
          if (!estado.bonus_interpretacion[pistaBonusId]) estado.bonus_interpretacion[pistaBonusId] = {};
          estado.bonus_interpretacion[pistaBonusId][jugIdx] = (estado.bonus_interpretacion[pistaBonusId][jugIdx] || 0) + ef.valor;
